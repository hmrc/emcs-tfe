...
@Singleton()
class ExplainDelayUserAnswersController @Inject()(cc: ControllerComponents,
                                                  userAnswersRepo: ExplainDelayUserAnswersService,
  ...
  def set(ern: String, arc: String): Action[JsValue] =
    authorisedUserSubmissionRequest(ern) {
      implicit request =>
        withJsonBody[ExplainDelayUserAnswers] {
          answers =>
