...
class UserAllowListController @Inject()(cc: ControllerComponents,
  ...
  def checkEligibility(ern: String, serviceName: String): Action[AnyContent] = authorisedUserRequest(ern) { implicit request =>
    ...
    val listOfErnsToExclude = appConfig.listOfErnsToExcludeFromPublicBeta

    if(listOfErnsToExclude.contains(ern)) {
      logger.debug(s"[checkEligibility] User $ern is specifically excluded from public beta")
      Future(NoContent)
    } else {
