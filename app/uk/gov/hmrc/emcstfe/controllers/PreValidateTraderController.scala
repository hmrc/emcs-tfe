...
class PreValidateTraderController @Inject() (cc: ControllerComponents, val config: AppConfig, override val auth: AuthAction, preValidateTraderService: PreValidateTraderService)(implicit ec: ExecutionContext)
    extends BackendController(cc)
    with AuthActionHelper
    with Logging
    with FeatureSwitching {

  def submit(ern: String): Action[JsValue] = authorisedUserSubmissionRequest(ern) { implicit request =>
    withJsonBody[PreValidateTraderModel] { model =>
      preValidateTraderService
        .preValidateTrader(model)
