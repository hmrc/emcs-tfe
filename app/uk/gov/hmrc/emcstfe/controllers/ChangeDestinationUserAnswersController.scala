...
@Singleton()
class ChangeDestinationUserAnswersController @Inject()(cc: ControllerComponents,
                                                       userAnswersRepo: ChangeDestinationUserAnswersService,
  ...
  def set(ern: String, arc: String): Action[JsValue] =
    authorisedUserSubmissionRequest(ern) {
      implicit request =>
        withJsonBody[ChangeDestinationUserAnswers] {
          answers =>
