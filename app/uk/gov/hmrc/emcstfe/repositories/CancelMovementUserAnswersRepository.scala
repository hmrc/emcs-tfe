...
@Singleton
class CancelMovementUserAnswersRepository @Inject()(
  ...
  extends PlayMongoRepository[CancelMovementUserAnswers](
    ...
    indexes        = Seq(
      IndexModel(
        ...
        IndexOptions()
          .name("lastUpdatedIdx")
          .expireAfter(appConfig.cancelAMovementUserAnswersTTL().toSeconds, TimeUnit.SECONDS)
      ),
      IndexModel(
  ...
  def get(internalId: String, ern: String, arc: String): Future[Option[CancelMovementUserAnswers]] =
