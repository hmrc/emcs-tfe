...
@Singleton
class ExplainShortageOrExcessUserAnswersRepository @Inject()(
  ...
  extends PlayMongoRepository[ExplainShortageOrExcessUserAnswers](
    ...
    indexes        = Seq(
      IndexModel(
        ...
        IndexOptions()
          .name("lastUpdatedIdx")
          .expireAfter(appConfig.explainShortageOrExcessUserAnswersTTL().toSeconds, TimeUnit.SECONDS)
      ),
      IndexModel(
  ...
  def get(internalId: String, ern: String, arc: String): Future[Option[ExplainShortageOrExcessUserAnswers]] =
