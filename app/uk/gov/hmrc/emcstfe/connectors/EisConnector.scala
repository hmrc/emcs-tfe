...
class EisConnector @Inject() (val http: HttpClient, appConfig: AppConfig, override val metricsService: MetricsService) extends BaseEisConnector with EisJsonHttpParser with Logging {
  ...
  def getMessages(request: GetMessagesRequest)(implicit headerCarrier: HeaderCarrier, ec: ExecutionContext, jsonReads: Reads[GetMessagesResponse]): Future[Either[ErrorResponse, GetMessagesResponse]] =
    prepareGetRequestAndSubmit(appConfig.eisGetMessagesUrl(), request, "getMessages", appConfig.eisMessagesBearerToken)

  def getMessageStatistics(request: GetMessageStatisticsRequest)(implicit headerCarrier: HeaderCarrier, ec: ExecutionContext, jsonReads: Reads[GetMessageStatisticsResponse]): Future[Either[ErrorResponse, GetMessageStatisticsResponse]] = {
  ...
  def getRawMovement(request: GetMovementRequest)(implicit headerCarrier: HeaderCarrier, ec: ExecutionContext, jsonReads: Reads[RawGetMovementResponse]): Future[Either[ErrorResponse, RawGetMovementResponse]] = {
    prepareGetRequestAndSubmit(appConfig.eisGetMovementUrl(), request, "getMovement", appConfig.eisMovementsBearerToken)
  }

  def getMovementList(request: GetMovementListRequest)(implicit headerCarrier: HeaderCarrier, ec: ExecutionContext, jsonReads: Reads[GetMovementListResponse]): Future[Either[ErrorResponse, GetMovementListResponse]] = {
    prepareGetRequestAndSubmit(appConfig.eisGetMovementsUrl(), request, "getMovementList", appConfig.eisMovementsBearerToken)
  }

  def getMovementHistoryEvents(
      request: GetMovementHistoryEventsRequest)(implicit headerCarrier: HeaderCarrier, ec: ExecutionContext, jsonReads: Reads[GetMovementHistoryEventsResponse]): Future[Either[ErrorResponse, GetMovementHistoryEventsResponse]] = {
  ...
  def getSubmissionFailureMessage(
      request: GetSubmissionFailureMessageRequest)(implicit headerCarrier: HeaderCarrier, ec: ExecutionContext, jsonReads: Reads[GetSubmissionFailureMessageResponse]): Future[Either[ErrorResponse, GetSubmissionFailureMessageResponse]] =
  ...
  def markMessageAsRead(request: MarkMessageAsReadRequest)(implicit headerCarrier: HeaderCarrier, ec: ExecutionContext, jsonReads: Reads[MarkMessageAsReadResponse]): Future[Either[ErrorResponse, MarkMessageAsReadResponse]] = {
  ...
  def setMessageAsLogicallyDeleted(
      request: SetMessageAsLogicallyDeletedRequest)(implicit headerCarrier: HeaderCarrier, ec: ExecutionContext, jsonReads: Reads[SetMessageAsLogicallyDeletedResponse]): Future[Either[ErrorResponse, SetMessageAsLogicallyDeletedResponse]] = {
    val url = appConfig.eisMessageUrl(request.exciseRegistrationNumber, request.messageId)
    prepareDeleteRequestAndSubmit(url, request, "setMessageAsLogicallyDeleted", appConfig.eisMessagesBearerToken)
  }

  def preValidateTrader(request: PreValidateRequest)(implicit headerCarrier: HeaderCarrier, ec: ExecutionContext, jsonReads: Reads[PreValidateTraderApiResponse]): Future[Either[ErrorResponse, PreValidateTraderApiResponse]] = {
  ...
  def preValidateTraderViaETDS12(request: PreValidateETDS12Request)(implicit headerCarrier: HeaderCarrier, ec: ExecutionContext, jsonReads: Reads[PreValidateTraderETDSResponse]): Future[Either[ErrorResponse, PreValidateTraderETDSResponse]] = {
  ...
  def getTraderKnownFactsViaETDS18(ern: String)(implicit request: UserRequest[_], headerCarrier: HeaderCarrier, ec: ExecutionContext, jsonReads: Reads[TraderKnownFacts]): Future[Either[ErrorResponse, Option[TraderKnownFacts]]] = {
    val url = appConfig.knownFactsEtdsUrl(ern)
    prepareGetRequestAndSubmit(url, TraderKnownFactsETDS18Request(request), "getKnownFactsViaETDS18", appConfig.eisTraderKnownFactsETDS18BearerToken).map(_.map(Some(_)))
  }
