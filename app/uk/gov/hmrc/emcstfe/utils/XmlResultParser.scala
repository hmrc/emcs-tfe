...
object XmlResultParser extends Logging {
  ...
  def parseResult[A]: ParseResult[A] => Either[ErrorResponse, A] = {
    ...
    case ParseFailure(errors) =>
      logger.warn(s"[handleParseResult] XML Response could not be parsed to model. Errors: \n\n - ${errors.mkString("\n - ")}")
      Left(XmlParseError(errors))
    case PartialParseSuccess(_, errors) =>
      logger.warn(s"[handleParseResult] PartialParseSuccess - XML Response could not be fully parsed to model. Errors: \n\n - ${errors.mkString("\n - ")}")
      Left(XmlParseError(errors))
  }
}
