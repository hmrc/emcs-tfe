    ...
    new QueryStringBindable[GetMovementListSearchOptions] {
      ...
      override def unbind(key: String, searchOptions: GetMovementListSearchOptions): String =
        Seq(
          ...
          searchOptions.dateOfReceiptFrom.map(stringBinder.unbind(key + ".dateOfDispatchFrom", _)),
          searchOptions.dateOfReceiptTo.map(stringBinder.unbind(key + ".dateOfDispatchTo", _)),
