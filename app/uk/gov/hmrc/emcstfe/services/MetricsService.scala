...
class MetricsService @Inject() (metricsRegistry: MetricRegistry) extends Logging {

  def requestTimer(metricName: String): Timer = metricsRegistry.timer(s"$metricName.timer")

  def rorStatusCounter(status: String) = metricsRegistry.counter(s"report-receipt.status-count.$status")
