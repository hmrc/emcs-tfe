...
class SubmitCreateMovementService @Inject()(chrisConnector: ChrisConnector,
  ...
  private def handleResponse[A](requestModel: SubmitCreateMovementRequest, response: Either[ErrorResponse, A])
    ...
    case Left(rimError: EISRIMValidationError) =>
      logger.warn(s"[handleResponse] - RIM validation error codes for correlation ID - ${requestModel.exciseRegistrationNumber} - ${rimError.errorResponse.emcsCorrelationId}: ${rimError.errorResponse.validatorResults.map(_.flatMap(formatErrorForLogging))}")
    ...
    case Left(rimError: ChRISRIMValidationError) =>
      logger.warn(s"[handleResponse] - RIM validation error codes for correlation ID - ${requestModel.exciseRegistrationNumber} - ${requestModel.correlationUUID}: ${rimError.errorResponse.rimValidationErrors.map(formatErrorForLogging)}")
      createMovementUserAnswersRepository.setValidationErrorMessagesForDraftMovement(
        requestModel.exciseRegistrationNumber,
        requestModel.draftId,
