...
import uk.gov.hmrc.emcstfe.mocks.services.MockCancelMovementUserAnswersService
...
class CancelMovementUserAnswersControllerSpec extends UnitSpec with MockCancelMovementUserAnswersService with FakeAuthAction {

  private val fakeRequest = FakeRequest("GET", "/user-answers/cancel-movement/:ern/:arc")
  private val controller = new CancelMovementUserAnswersController(
  ...
  "PUT /user-answers/cancel-movement/:ern/:arc" should {
    s"return $OK (OK)" when {
    ...
    s"return $BAD_REQUEST (BAD_REQUEST)" when {
      "Received JSON cannot be parsed to CancelMovementUserAnswers" in {

        val result = controller.set(testErn, testArc)(fakeRequest.withBody(Json.obj()))

        status(result) shouldBe Status.BAD_REQUEST
        contentAsString(result) should include("Invalid CancelMovementUserAnswers payload")
      }
    }
    s"return $INTERNAL_SERVER_ERROR (ISE)" when {
      "service returns a Left" in {
  ...
  "DELETE /user-answers/cancel-movement/:ern/:arc" should {
    s"return $NO_CONTENT (NO_CONTENT)" when {
