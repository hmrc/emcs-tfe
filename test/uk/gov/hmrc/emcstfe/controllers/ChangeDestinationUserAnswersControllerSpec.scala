...
import uk.gov.hmrc.emcstfe.mocks.services.MockChangeDestinationUserAnswersService
...
class ChangeDestinationUserAnswersControllerSpec extends UnitSpec with MockChangeDestinationUserAnswersService with FakeAuthAction {

  private val fakeRequest = FakeRequest("GET", "/user-answers/change-destination/:ern/:arc")
  private val controller = new ChangeDestinationUserAnswersController(
  ...
  val userAnswers = ChangeDestinationUserAnswers(testErn, testArc, Json.obj(), Instant.now().truncatedTo(ChronoUnit.MILLIS))

  "GET /user-answers/change-destination/:ern/:arc" should {
    s"return $OK (OK)" when {
  ...
  "PUT /user-answers/change-destination/:ern/:arc" should {
    s"return $OK (OK)" when {
    ...
    s"return $BAD_REQUEST (BAD_REQUEST)" when {
      "Received JSON cannot be parsed to ChangeDestinationUserAnswers" in {

        val result = controller.set(testErn, testArc)(fakeRequest.withBody(Json.obj()))

        status(result) shouldBe Status.BAD_REQUEST
        contentAsString(result) should include("Invalid ChangeDestinationUserAnswers payload")
      }
    }
    s"return $INTERNAL_SERVER_ERROR (ISE)" when {
      "service returns a Left" in {
  ...
  "DELETE /user-answers/change-destination/:ern/:arc" should {
    s"return $NO_CONTENT (NO_CONTENT)" when {
