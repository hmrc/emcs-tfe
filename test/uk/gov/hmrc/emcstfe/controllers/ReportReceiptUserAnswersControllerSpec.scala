...
class ReportReceiptUserAnswersControllerSpec extends UnitSpec with MockReportReceiptUserAnswersService with FakeAuthAction {
  ...
  private val fakeRequest = FakeRequest("GET", "/user-answers/report-receipt/:ern/:arc")
  ...
  "PUT /user-answers/report-receipt/:ern/:arc" should {
    s"return $OK (OK)" when {
    ...
    s"return $BAD_REQUEST (BAD_REQUEST)" when {
      "Received JSON cannot be parsed to ReportReceiptUserAnswers" in {

        val result = controller.set(testErn, testArc)(fakeRequest.withBody(Json.obj()))

        status(result) shouldBe Status.BAD_REQUEST
        contentAsString(result) should include("Invalid ReportReceiptUserAnswers payload")
      }
    }
    s"return $INTERNAL_SERVER_ERROR (ISE)" when {
      "service returns a Left" in {
  ...
  "DELETE /user-answers/report-receipt/:ern/:arc" should {
    s"return $NO_CONTENT (NO_CONTENT)" when {
