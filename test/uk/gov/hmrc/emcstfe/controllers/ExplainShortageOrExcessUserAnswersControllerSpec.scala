...
import uk.gov.hmrc.emcstfe.mocks.services.MockShortageOrExcessUserAnswersService
...
class ExplainShortageOrExcessUserAnswersControllerSpec extends UnitSpec with MockShortageOrExcessUserAnswersService with FakeAuthAction {

  private val fakeRequest = FakeRequest("GET", "/user-answers/explain-delay/:ern/:arc")
  private val controller = new ExplainShortageOrExcessUserAnswersController(
  ...
  "PUT /user-answers/explain-shortage-or-excess/:ern/:arc" should {
    s"return $OK (OK)" when {
    ...
    s"return $BAD_REQUEST (BAD_REQUEST)" when {
      "Received JSON cannot be parsed to ExplainShortageOrExcessUserAnswers" in {

        val result = controller.set(testErn, testArc)(fakeRequest.withBody(Json.obj()))

        status(result) shouldBe Status.BAD_REQUEST
        contentAsString(result) should include("Invalid ExplainShortageOrExcessUserAnswers payload")
      }
    }
    s"return $INTERNAL_SERVER_ERROR (ISE)" when {
      "service returns a Left" in {
  ...
  "DELETE /user-answers/explain-shortage-or-excess/:ern/:arc" should {
    s"return $NO_CONTENT (NO_CONTENT)" when {
