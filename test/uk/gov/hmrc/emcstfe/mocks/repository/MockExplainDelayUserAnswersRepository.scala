...
trait MockExplainDelayUserAnswersRepository extends MockitoSugar  {

  lazy val mockRepo: ExplainDelayUserAnswersRepository = mock[ExplainDelayUserAnswersRepository]

  object MockUserAnswers {
    def set(answers: ExplainDelayUserAnswers): OngoingStubbing[Future[Boolean]] =
      when(mockRepo.set(eqTo(answers)))

    def get(ern: String, arc: String): OngoingStubbing[Future[Option[ExplainDelayUserAnswers]]] =
