...
class WineProductModelSpec extends CreateMovementSpec with CreateMovementFixtures {
  ...
  testJsonToModelToXml(
    scenario = "WineProductModel with max fields",
    json = wineProductJsonMax,
    model = wineProductModelMax,
    xml = wineProductXmlMax
  )

  testJsonToModelToXml(
    scenario = "WineProductModel with min fields",
    json = wineProductJsonMin,
    model = wineProductModelMin,
    xml = wineProductXmlMin
  )
}
