...
class SubmitChangeOfDestinationRequestSpec extends UnitSpec with SubmitChangeOfDestinationFixtures {
  ...
  "requestBody" should {
        ...
        <soapenv:Envelope xmlns:soapenv="http://www.w3.org/2003/05/soap-envelope">
          ...
          <soapenv:Body>
            <urn:IE813 xmlns:urn="urn:publicid:-:EC:DGTAXUD:EMCS:PHASE4:IE813:V3.01" xmlns:urn1="urn:publicid:-:EC:DGTAXUD:EMCS:PHASE4:TMS:V3.01">
              ...
              <urn:Body>
                {submitChangeOfDestinationXmlMax}
              </urn:Body>
            </urn:IE813>
          </soapenv:Body>
        </soapenv:Envelope>
    ...
    "for the MessageSender and MessageRecipient headers" when {
      ...
      val model =
        submitChangeOfDestinationModelMax
      ...
      "generating MessageSender" should {
        "use the country code from the ARC" in {
          val request = SubmitChangeOfDestinationRequest(model)
      ...
      "generating MessageRecipient" when {
        "destination type is TaxWarehouse" should {
          "use the newConsigneeTrader traderId for the Country Code" in {
            val request = SubmitChangeOfDestinationRequest(model.copy(destinationChanged = model.destinationChanged.copy(destinationTypeCode = TaxWarehouse)))
            request.messageRecipient shouldBe "NDEA.FR"
          }

          "use GB as default when newConsigneeTrader does NOT exist" in {
            val request = SubmitChangeOfDestinationRequest(model.copy(destinationChanged = model.destinationChanged.copy(destinationTypeCode = TaxWarehouse, newConsigneeTrader = None)))
            request.messageRecipient shouldBe "NDEA.GB"
          }

          "use GB as default when newConsigneeTrader traderId does NOT exist" in {
            val request = SubmitChangeOfDestinationRequest(model.copy(destinationChanged = model.destinationChanged.copy(destinationTypeCode = TaxWarehouse, newConsigneeTrader = Some(newConsigneeTraderModelMin))))
            request.messageRecipient shouldBe "NDEA.GB"
          }
        }
        ...
        "destination type is Export" should {
          "use the deliveryPlaceCustomsOffice referenceNumber for the Country Code when it exists" in {
            val request = SubmitChangeOfDestinationRequest(model.copy(destinationChanged = model.destinationChanged.copy(destinationTypeCode = Export)))
            request.messageRecipient shouldBe "NDEA.IT"
          }

          "use GB as default when deliveryPlaceCustomsOffice does NOT exist" in {

            val request = SubmitChangeOfDestinationRequest(model.copy(destinationChanged = model.destinationChanged.copy(destinationTypeCode = Export, deliveryPlaceCustomsOffice = None)))
            request.messageRecipient shouldBe "NDEA.GB"
          }
        }
        ...
        "destination type is anything else" should {
          "use GB" in {
            val request = SubmitChangeOfDestinationRequest(model.copy(destinationChanged = model.destinationChanged.copy(destinationTypeCode = ReturnToThePlaceOfDispatchOfTheConsignor)))
            request.messageRecipient shouldBe "NDEA.GB"
          }
        }
      }
