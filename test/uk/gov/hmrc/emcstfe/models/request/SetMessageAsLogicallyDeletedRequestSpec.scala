...
class SetMessageAsLogicallyDeletedRequestSpec extends TestBaseSpec {

  val request = SetMessageAsLogicallyDeletedRequest(exciseRegistrationNumber = testErn, messageId = testMessageId)
  ...
  "requestBody" should {
    "generate the correct request XML" in {
      ...
      (xml \\ "Envelope" \\ "Body" \\ "Control" \\"OperationRequest" \\ "Parameters" \\ "Parameter").filter(el => (el \ "@Name").text == "UniqueMessageId").text shouldBe testMessageId
    }
  }

  "action" should {
    "be correct" in {
      request.action shouldBe "http://www.govtalk.gov.uk/taxation/internationalTrade/Excise/EMCSApplicationService/2.0/SetMessageAsLogicallyDeleted"
    }
  }
  ...
  "queryParams" should {
    "return the ERN as a query param" in {
