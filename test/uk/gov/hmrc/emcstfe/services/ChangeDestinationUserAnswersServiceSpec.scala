...
class ChangeDestinationUserAnswersServiceSpec extends UnitSpec with GetMovementListFixture {
  trait Test extends MockChangeDestinationUserAnswersRepository {
    val service: ChangeDestinationUserAnswersService = new ChangeDestinationUserAnswersService(mockRepo)
  }

  val userAnswers = ChangeDestinationUserAnswers(testErn, testArc, Json.obj(), Instant.now())
