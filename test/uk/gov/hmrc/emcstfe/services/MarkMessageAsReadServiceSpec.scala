...
class MarkMessageAsReadServiceSpec extends TestBaseSpec with MarkMessageAsReadFixtures {

  trait Test extends MockEisConnector {
    val markMessageAsReadRequest: MarkMessageAsReadRequest = MarkMessageAsReadRequest(testErn, testMessageId)
    val service: MarkMessageAsReadService = new MarkMessageAsReadService(mockEisConnector)
  }
