...
class GetMovementHistoryEventsServiceSpec extends TestBaseSpec with GetMovementHistoryEventsFixture {
  ...
  trait Test extends MockEisConnector with MockAppConfig {
    ...
    val service: GetMovementHistoryEventsService = new GetMovementHistoryEventsService(mockEisConnector, mockAppConfig)
  }
  ...
  "getMovementHistoryEvent" should {
    "return Right" when {
      "request is valid and connector call succeeds" in new Test {
        ...
        await(service.getMovementHistoryEvent(movementHistoryEventsRequest)) shouldBe Right(getMovementHistoryEventsResponseModel)
      }
    }
