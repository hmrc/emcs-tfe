...
import uk.gov.hmrc.emcstfe.mocks.repository.MockCancelMovementUserAnswersRepository
...
class CancelMovementUserAnswersServiceSpec extends UnitSpec with GetMovementListFixture {
  trait Test extends MockCancelMovementUserAnswersRepository {
    val service: CancelMovementUserAnswersService = new CancelMovementUserAnswersService(mockRepo)
  }

  val userAnswers = CancelMovementUserAnswers(testInternalId, testErn, testArc, Json.obj(), Instant.now())
