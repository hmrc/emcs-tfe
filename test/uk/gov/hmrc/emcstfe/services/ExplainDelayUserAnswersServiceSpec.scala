...
import uk.gov.hmrc.emcstfe.mocks.repository.MockExplainDelayUserAnswersRepository
...
class ExplainDelayUserAnswersServiceSpec extends UnitSpec with GetMovementListFixture {
  trait Test extends MockExplainDelayUserAnswersRepository {
    val service: ExplainDelayUserAnswersService = new ExplainDelayUserAnswersService(mockRepo)
  }

  val userAnswers = ExplainDelayUserAnswers(testInternalId, testErn, testArc, Json.obj(), Instant.now())
