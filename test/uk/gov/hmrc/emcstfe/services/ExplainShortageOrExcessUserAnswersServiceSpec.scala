...
import uk.gov.hmrc.emcstfe.mocks.repository.MockExplainShortageOrExcessUserAnswersRepository
...
class ExplainShortageOrExcessUserAnswersServiceSpec extends UnitSpec with GetMovementListFixture {
  trait Test extends MockExplainShortageOrExcessUserAnswersRepository {
    val service: ExplainShortageOrExcessUserAnswersService = new ExplainShortageOrExcessUserAnswersService(mockRepo)
  }

  val userAnswers = ExplainShortageOrExcessUserAnswers(testInternalId, testErn, testArc, Json.obj(), Instant.now())
