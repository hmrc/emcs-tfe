...
class GetMessagesIntegrationSpec extends IntegrationBaseSpec with GetMessagesFixtures {
  ...
  private trait Test {
    ...
    def downstreamUri: String = s"/emcs/messages/v1/messages"
    ...
    "user is authorised" when {
      ...
      "return a success" when {
        "all downstream calls are successful" in new Test {
          override def setupStubs(): StubMapping = {
            AuthStub.authorised()
            DownstreamStub.onSuccess(DownstreamStub.GET, downstreamUri, downstreamQueryParams, Status.OK, getMessagesResponseDownstreamJson)
          }
      ...
      "return an error" when {
        "downstream call returns XML with the wrong encoding" in new Test {
          override def setupStubs(): StubMapping = {
            AuthStub.authorised()
            DownstreamStub.onSuccess(DownstreamStub.GET, downstreamUri, downstreamQueryParams, Status.OK, getMessagesResponseDownstreamJsonWrongEncoding)
          }
        ...
        "downstream call returns unencoded XML" in new Test {
          override def setupStubs(): StubMapping = {
            AuthStub.authorised()
            DownstreamStub.onSuccess(DownstreamStub.GET, downstreamUri, downstreamQueryParams, Status.OK, getMessagesResponseDownstreamJsonNotEncoded)
          }
        ...
        "downstream call returns XML which can't be parsed to JSON" in new Test {
          override def setupStubs(): StubMapping = {
            AuthStub.authorised()
            DownstreamStub.onSuccess(DownstreamStub.GET, downstreamUri, downstreamQueryParams, Status.OK, getMessagesResponseDownstreamJsonBadXml)
          }
