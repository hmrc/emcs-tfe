...
class ChangeDestinationUserAnswersRepositorySpec extends IntegrationBaseSpec
    with DefaultPlayMongoRepositorySupport[ChangeDestinationUserAnswers]
    with MockFactory
    with OptionValues
  ...
  private val userAnswers = ChangeDestinationUserAnswers(testErn, testArc, Json.obj("foo" -> "bar"), Instant.ofEpochSecond(1))
