...
class CancelMovementUserAnswersRepositorySpec extends IntegrationBaseSpec
    with DefaultPlayMongoRepositorySupport[CancelMovementUserAnswers]
    with MockFactory
    with OptionValues
  ...
  private val userAnswers = CancelMovementUserAnswers(testInternalId, testErn, testArc, Json.obj("foo" -> "bar"), Instant.ofEpochSecond(1))

  private val mockAppConfig = mock[AppConfig]
  (() => mockAppConfig.cancelAMovementUserAnswersTTL(): Duration)
    .expects()
    .returns(Duration("1seconds"))
    .anyNumberOfTimes()
  (() => mockAppConfig.cancelAMovementUserAnswersReplaceIndexes(): Boolean)
    .expects()
    .returns(true)
    .anyNumberOfTimes()

  protected override val repository = new CancelMovementUserAnswersRepository(
