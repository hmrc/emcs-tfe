...
class ExplainShortageOrExcessUserAnswersRepositorySpec extends IntegrationBaseSpec
    with DefaultPlayMongoRepositorySupport[ExplainShortageOrExcessUserAnswers]
    with MockFactory
    with OptionValues
  ...
  private val userAnswers = ExplainShortageOrExcessUserAnswers(testInternalId, testErn, testArc, Json.obj("foo" -> "bar"), Instant.ofEpochSecond(1))

  private val mockAppConfig = mock[AppConfig]
  (() => mockAppConfig.explainShortageOrExcessUserAnswersTTL(): Duration)
    .expects()
    .returns(Duration("1seconds"))
    .anyNumberOfTimes()
  (() => mockAppConfig.explainShortageOrExcessUserAnswersReplaceIndexes(): Boolean)
    .expects()
    .returns(true)
    .anyNumberOfTimes()

  protected override val repository = new ExplainShortageOrExcessUserAnswersRepository(
